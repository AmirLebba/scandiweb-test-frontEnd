...
import { ProductGridItemProps, Product } from "@interfaces/interfaces";
...
export const ProductGridItem: React.FC<ProductGridItemProps> = ({
  ...
  // Track hover state for each product card
          ...
          if (product.inStock) {
            ...
            product.attributes.forEach((attr) => {
              if (attr.items.length > 0) {
                defaultAttributes[attr.name] = attr.items[0].value; // Select first available option
              }
            });
